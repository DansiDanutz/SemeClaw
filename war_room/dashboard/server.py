...
from war_room.dashboard.routes.voice import _LANG_NAMES
...
async def _prune_agent_history(agent_name: str):
    ...
    try:
        rows = await _supa(
            "get",
            f"agent_run_history?agent_name=eq.{agent_name}&status=in.(success,failed)&order=created_at.desc&select=id",
        )
...
async def _start_background_tasks() -> list[asyncio.Task]:
    ...
    from war_room.dashboard.routes.fleet import droplet_probe_loop

    _startup_config_checks()
    tasks = [
        asyncio.create_task(file_watcher(), name="semeclaw-file-watcher"),
        asyncio.create_task(droplet_probe_loop(), name="semeclaw-droplet-probe-loop"),
...
# ---------------------------------------------------------------------------
try:
    from war_room.dashboard.routes import (
        advertiser,
        agents,
        alerts,
        billing,
...
_PRO_KEYS: set[str] = {k.strip() for k in os.environ.get("SEMECLAW_PRO_KEYS", "").split(",") if k.strip()}
...
def _get_tier(request: Request) -> str:
    ...
    if not _PRO_KEYS:
        return "free"
    key = (request.headers.get("x-semeclaw-license", "") or request.query_params.get("license", "")).strip()
...
# ---------------------------------------------------------------------------
_paperclip_company_id: str | None = None
PAPERCLIP_BASE = "http://127.0.0.1:3100"


async def _get_company_id() -> str | None:
...
# ---------------------------------------------------------------------------
# Report ingest helpers — the /api/reports endpoints live in routes/reports.py
# ---------------------------------------------------------------------------

import hashlib as _hash_ing
import hmac as _hmac_ing
import re as _re_ing
import time as _time_ing
import uuid as _uuid_ing
...
async def api_events(tenant: str | None = None, events: str | None = None):
    ...
    async def _stream():
        ...
        hello = {
            ...
            "ts": datetime.now(timezone.utc).isoformat(),
            "agent_version": APP_VERSION,
            "data": {"subscribers": len(_SSE_SUBSCRIBERS)},
        }
...
async def api_meeting_share(name: str):
    ...
    shares = _load_shares()
    shares[token] = {"name": safe, "expires": expires, "created": datetime.now(timezone.utc).isoformat()}
    _save_shares(shares)
    return JSONResponse(
        {
            "ok": True,
...
async def share_page(token: str):
    ...
    return FileResponse(
        index, media_type="text/html", headers={"X-SemeClaw-Share": token, "X-SemeClaw-Meeting": s["name"]}
    )
...
# ---------------------------------------------------------------------------
# v0.5.0 — Meeting templates, Cost ledger, Audit (voice overrides → routes/voice.py)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# v0.6.0 — SRT/PDF exports, Stripe billing hook (voice cloning → routes/voice.py)
# ---------------------------------------------------------------------------
...
async def api_meeting_finalize(request: Request):
    ...
    qa_text = "\n".join(
        f"Q: {qa.get('question', '')}\nA ({qa.get('responder', '')}): {qa.get('response', '')}" for qa in qa_pairs
    )
    system = (
...
async def api_spotlight_impression(request: Request):
    ...
    ip = (
        ((request.headers.get("x-forwarded-for") or "") or (request.client.host if request.client else "unknown"))
        .split(",")[0]
        .strip()
    )
                ...
                builds.append(
                    {
                        ...
                        "modified": datetime.fromtimestamp(index.stat().st_mtime, tz=timezone.utc).isoformat()
                        if index.exists()
                        else None,
                    }
                )
        ...
        for s in skills:
            if s["speaker"] in attendees:
                skill_summaries.append(
                    f"- {s['speaker']} ({s['role']}): {s['how_to_invoke']} Triggers: {', '.join(s['triggers'][:5])}"
                )
    ...
    return JSONResponse(
        {
            ...
            "input_schema": {
                ...
                "webhook_url": {"type": "string", "required": False, "desc": "Posts back meeting.finalized payload"},
            },
            "output_schema": {
                "meeting_id": "string",
                "report_name": "string",
