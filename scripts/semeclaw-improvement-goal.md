...
| # | Item | Finding | Status |
|---|------|---------|--------|
| 3.1 | Extract `APIRouter` modules from `server.py` by surface, one PR each, until it is app assembly + middleware only. Done: **voice** (tts/stt/voices → `routes/voice.py`, −550 lines, dead edge-tts era code dropped), **reports** (5 routes → `routes/reports.py`), **dead MOVED bodies** (`api_agent_manifest`/`api_board`, −200 lines; server.py at 5,161). Remaining surfaces by size: meeting (26 routes + its helpers), agent manifest/health (5), paperclip (4), webhooks (3), admin (3), misc | SC-05 | 🟡 in progress |
