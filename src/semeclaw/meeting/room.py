...
class MeetingRoom:
    ...
    def __init__(self, topic: str, agent_ids: list[str], config: Config, engine: str = "native"):
        self.topic = topic
    ...
    async def _run_crewai_meeting(self):
        ...
        for aid in self.agent_ids:
            agent = build_crewai_agent(aid)
            task = build_crewai_task(
                agent=agent,
                agent_id=aid,
                original_task=f"Strategy meeting about: {self.topic}",
            )
        ...
        for idx, task in enumerate(crew_tasks):
            ...
            display = output.replace("\n", " ")[:200] + "..." if len(output) > 200 else output
            print(f"  [{aid}] {display}\n")
            agent_outputs.append({"agent": aid, "response": output})

        # Use Crew's final output as summary if available, else synthesize
        summary = str(crew.output) if crew.output else await self._synthesize(agent_outputs, mock=False)
        return agent_outputs, summary
    ...
    async def generate_context(self, mock: bool = False) -> Path:
        ...
        if self.engine == "crewai" and not mock:
            agent_outputs, summary = await self._run_crewai_meeting()
        else:
